...
class Trainer:
    ...
    # @torch.no_grad()
    # def _generate_set(self, mode, path):
    #     print('loading test dataset, the number is', len(self.data_loader))
    #     self.model.eval()
    #     # use the first batch of dataloader in all validations for better visualization comparisons
    #     data_loader_iter = iter(self.data_loader)
    #     train_data = next(data_loader_iter)
    #     # prepare input
    #     images, style_ref, laplace_ref, content_ref = train_data['img'].to(self.device), \
    #                                                   train_data['style'].to(self.device), \
    #                                                   train_data['laplace'].to(self.device), \
    #                                                   train_data['content'].to(self.device)
    #
    #     # load_content = ContentData()
    #     # forward
    #
    #
    #     for i_arc in content_ref:
    #         rank = dist.get_rank()
    #
    #         text_ref = i_arc
    #         text_ref = text_ref.to(self.device).repeat(style_ref.shape[0], 1, 1, 1)
    #         x = torch.randn((text_ref.shape[0], 4, 32, 32)).to(self.device)
    #         preds = self.diffusion.ddim_sample(self.model, self.vae, images.shape[0], x, style_ref, laplace_ref,
    #                                            text_ref)
    #         out_path = os.path.join(path, f"epoch-{epoch}-{65}-process-{rank}.png")
    #         self._save_images(preds, out_path)
    #
